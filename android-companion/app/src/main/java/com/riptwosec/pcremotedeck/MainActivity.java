...
public class MainActivity extends Activity implements WearBridge.Listener, WifiReconManager.Listener, CommandRouter.Listener {
    ...
    private void buildUi(){
        ...
        macroSteps=input("Steps comma-separated: APP_VSCODE,APP_CHROME,DEEP_FOCUS");root.addView(macroSteps);
    ...
    private void restorePcLink(){
        String h=prefs.getString("host","192.168.1.10"),t=prefs.getString("token","CHANGE_ME");
        host.setText(h);token.setText(t);pc.configure(h,8765,t);
        if(pc.isConfigured())status("PC LINK RESTORED");
    }
    private void savePcLink(){
        String h=host.getText().toString().trim(),t=token.getText().toString().trim();
        pc.configure(h,8765,t);prefs.edit().putString("host",h).putString("token",t).apply();status("PC LINK SAVED");refreshDashboard(true);
    }
    @Override public void revokePcLink(){
        prefs.edit().remove("host").remove("token").apply();host.setText("192.168.1.10");token.setText("CHANGE_ME");pc.configure("192.168.1.10",8765,"CHANGE_ME");lastDashboard=null;status("PC LINK REVOKED LOCALLY");
    }
    ...
    private void handleVoice(String speech){
        ...
        if(has(s,"battle","เกม","gaming"))action="BATTLE_STATION";
        else if(has(s,"focus","โฟกัส","ทำงาน"))action="DEEP_FOCUS";
        else if(has(s,"lock","ล็อก"))action="PC_LOCK";
        else if(has(s,"mute","ปิดเสียง"))action="MEDIA_MUTE";
        else if(has(s,"volume up","เพิ่มเสียง"))action="VOLUME_UP";
        else if(has(s,"volume down","ลดเสียง"))action="VOLUME_DOWN";
        else if(has(s,"chrome","โครม"))action="APP_CHROME";
        else if(has(s,"vs code","visual studio code","วีเอสโค้ด"))action="APP_VSCODE";
        else if(has(s,"steam","สตีม"))action="APP_STEAM";
        else if(has(s,"discord","ดิสคอร์ด"))action="APP_DISCORD";
        else if(has(s,"screenshot","แคปหน้าจอ","จับภาพ"))action="SCREENSHOT";
        else if(has(s,"desktop","เดสก์ท็อป"))action="SHOW_DESKTOP";
        else if(has(s,"next","เพลงต่อไป"))action="MEDIA_NEXT";
        else if(has(s,"previous","เพลงก่อน"))action="MEDIA_PREV";
        else if(has(s,"play","pause","เล่นเพลง","หยุดเพลง"))action="MEDIA_PLAY_PAUSE";
        else if(has(s,"network","เน็ตเวิร์ก","เครือข่าย"))action="GET_NETWORK";
        else if(has(s,"monitor","cpu","สถานะคอม"))action="GET_DASHBOARD";
        else if(has(s,"window","หน้าต่าง"))action="GET_WINDOWS";
        else if(has(s,"audio","มิกเซอร์"))action="GET_AUDIO";
        else if(has(s,"macro work","มาโครทำงาน")){runMacroVoice("work",speech);return;}
        else if(has(s,"macro game","มาโครเกม")){runMacroVoice("game",speech);return;}
        else if(has(s,"shutdown","ปิดคอม")){confirmRisk("SYSTEM_SHUTDOWN","Shutdown PC?",speech);return;}
        else if(has(s,"restart","รีสตาร์ต")){confirmRisk("SYSTEM_RESTART","Restart PC?",speech);return;}
        else if(has(s,"sleep","สลีป")){confirmRisk("SYSTEM_SLEEP","Sleep PC?",speech);return;}
    ...
    private void sendVoiceResult(String action,String speech,JSONObject payload){
        try{JSONObject r=new JSONObject();r.put("type","result");r.put("ok",action!=null);r.put("action","VOICE_PTT");r.put("message",action==null?"VOICE INTENT UNKNOWN":"VOICE: "+action);r.put("data",speech);wear.sendJson(r.toString());if(action!=null){JSONObject e=new JSONObject();e.put("id","voice-"+System.currentTimeMillis());e.put("action",action);e.put("payload",payload==null?new JSONObject():payload);router.route(e);}}catch(Exception ignored){}
    }

    private void refreshDashboard(boolean verbose){
        if(!pc.isConfigured()){if(verbose)status("PC LINK NOT CONFIGURED");return;}
        pc.command("GET_DASHBOARD",new JSONObject(),(ok,msg,body)->{if(ok){JSONObject d=body.optJSONObject("data");if(d!=null)lastDashboard=d;sendSnapshot();if(verbose)runOnUiThread(()->status("DASHBOARD SYNC OK"));}else if(verbose)runOnUiThread(()->status("PC DASHBOARD: "+msg));});
    }
    private void forwardPcResult(String action,JSONObject payload){
        if(!pc.isConfigured()){status("PC LINK NOT CONFIGURED");return;}
        pc.command(action,payload,(ok,msg,body)->{try{JSONObject r=new JSONObject();r.put("v",1);r.put("type","result");r.put("action",action);r.put("ok",ok);r.put("message",msg);JSONObject d=body.optJSONObject("data");if(d!=null)r.put("data",d);sendToWatch(r);runOnUiThread(()->status(msg));}catch(Exception ignored){}});
    }

    private void saveMacro(){
        String name=macroName.getText().toString().trim();String raw=macroSteps.getText().toString().trim();
        if(name.length()==0||raw.length()==0){status("MACRO NAME/STEPS REQUIRED");return;}
        String id=name.toLowerCase(Locale.ROOT).replaceAll("[^a-z0-9_-]+","_");
        JSONArray steps=new JSONArray();for(String part:raw.split(",")){String a=part.trim().toUpperCase(Locale.ROOT);if(a.length()>0)steps.put(a);}try{JSONObject p=new JSONObject();p.put("id",id);p.put("name",name);p.put("steps",steps);forwardPcResult("MACRO_SAVE",p);}catch(Exception e){status("MACRO ERROR");}
    }

    private void sendSnapshot(){
        try{
            JSONObject snap=new JSONObject();snap.put("type","snapshot");snap.put("ts",System.currentTimeMillis());snap.put("pc",lastDashboard!=null?lastDashboard:new JSONObject().put("state",pc.isConfigured()?"CONFIGURED":"NOT CONFIGURED"));
            JSONObject compact=new JSONObject();
            if(lastWifi!=null){compact.put("summary",lastWifi.optString("summary","WI-FI READY"));compact.put("scanState",lastWifi.optString("scanState","UNKNOWN"));compact.put("timestamp",lastWifi.optLong("timestamp",System.currentTimeMillis()));compact.put("nearby",lastWifi.optInt("nearby",0));compact.put("freeVerified",lastWifi.optInt("freeVerified",0));compact.put("open",lastWifi.optInt("open",0));compact.put("secured",lastWifi.optInt("secured",0));compact.put("loginRequired",lastWifi.optInt("loginRequired",0));JSONObject best=lastWifi.optJSONObject("best");if(best!=null){JSONObject bb=new JSONObject();bb.put("ssid",best.optString("ssid","UNKNOWN"));bb.put("status",best.optString("status","UNKNOWN"));bb.put("signalDbm",best.optInt("signalDbm",-127));bb.put("band",best.optString("band",""));bb.put("security",best.optString("security","UNKNOWN"));bb.put("score",best.optInt("score",0));bb.put("trend",best.optString("trend","STABLE"));compact.put("best",bb);}}else{compact.put("summary","NO SCAN");compact.put("scanState","UNKNOWN");}
            snap.put("wifi",compact);wear.sendJson(snap.toString());

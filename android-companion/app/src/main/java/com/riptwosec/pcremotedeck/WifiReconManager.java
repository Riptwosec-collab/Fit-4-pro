...
public final class WifiReconManager {
    ...
    public void start() {
        ...
        if (!ok) { status("SCAN LIMITED — USING RECENT RESULTS"); publish(false); }
    ...
    private void publish(boolean fresh) {
        try {
            ...
            for (ScanResult r : raw) {
                ...
                ScanResult old = strongest.get(ssid);
                if (old == null || r.level > old.level) strongest.put(ssid, r);
            }
            List<ScanResult> list = new ArrayList<>(strongest.values());
            Collections.sort(list, new Comparator<ScanResult>() { @Override public int compare(ScanResult a, ScanResult b) { return Integer.compare(b.level, a.level); } });

            String connectedSsid = currentSsid();
            CurrentValidation cv = currentValidation();
            JSONArray networks = new JSONArray();
            int open=0, secured=0, login=0, verified=0;
            int limit = Math.min(12, list.size());
            for (int n=0;n<limit;n++) {
                ScanResult r=list.get(n); String ssid=ssidOf(r); if(ssid.length()==0)ssid="<hidden>";
                String security=securityOf(r); boolean isOpen="OPEN".equals(security);
                if(isOpen) open++; else secured++;
                String status = isOpen ? "OPEN" : "SECURED";
                String internet="NOT TESTED";
                // Only the currently connected network is allowed to become VERIFIED/LOGIN/NO INTERNET.
                if (sameSsid(ssid, connectedSsid)) {
                    if (cv.captive) { status="LOGIN REQUIRED"; internet="CAPTIVE PORTAL"; login++; }
                    else if (cv.validated) { status="FREE VERIFIED"; internet="AVAILABLE"; verified++; }
                    else { status="NO INTERNET"; internet="NOT VALIDATED"; }
                }
                int score=score(r.level,security,status);
                JSONObject j=new JSONObject();
                j.put("ssid",ssid); j.put("signalDbm",r.level); j.put("signalLevel",signalLabel(r.level));
                j.put("band",bandOf(r.frequency)); j.put("frequency",r.frequency); j.put("security",security);
                j.put("status",status); j.put("internet",internet); j.put("score",score); j.put("live",fresh);
                networks.put(j);
            }
            JSONObject out=new JSONObject();
            out.put("timestamp",System.currentTimeMillis()); out.put("scanState",fresh?"READY":"CACHED");
            out.put("summary",list.size()+" nearby • "+verified+" verified • "+open+" open • "+secured+" secured");
            out.put("nearby",list.size()); out.put("freeVerified",verified); out.put("open",open); out.put("secured",secured); out.put("loginRequired",login);
            out.put("networks",networks);
            JSONObject best=null;
            for(int i=0;i<networks.length();i++){
                JSONObject candidate=networks.optJSONObject(i);
                if(candidate==null)continue;
                if(best==null || candidate.optInt("score",0)>best.optInt("score",0))best=candidate;
            }
            if(best!=null)out.put("best",best);
            if(listener!=null)listener.onRecon(out);
        } catch(Exception e){status("WI-FI ERROR: "+e.getMessage());}
    }

    private int score(int rssi,String security,String status){
        int signal = rssi>=-50?95:rssi>=-60?82:rssi>=-70?68:rssi>=-80?48:25;
        int sec="WPA3".equals(security)?95:"WPA2/WPA3".equals(security)?90:"WPA2".equals(security)?82:"ENTERPRISE".equals(security)?88:"OWE".equals(security)?75:28;
        int net="FREE VERIFIED".equals(status)?100:"LOGIN REQUIRED".equals(status)?55:"NO INTERNET".equals(status)?0:50;
        return Math.max(0,Math.min(100,(signal*45+sec*30+net*25)/100));
    }

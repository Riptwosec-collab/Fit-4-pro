...
public final class PcBridgeClient {
    ...
    public synchronized void configure(String host,int port,String token){this.baseUrl="http://"+host+":"+port;this.token=token==null?"CHANGE_ME":token.trim();}
    public synchronized boolean isConfigured(){return token.length()>8&&!"CHANGE_ME".equals(token);}
    public void command(final String action, final JSONObject payload, final Callback cb){
        final String url; final String secret;
        synchronized(this){url=baseUrl+"/command";secret=token;}
        new Thread(new Runnable(){@Override public void run(){
            HttpURLConnection c=null;
            try{
                URL u=new URL(url);c=(HttpURLConnection)u.openConnection();c.setConnectTimeout(2500);c.setReadTimeout(7000);c.setRequestMethod("POST");c.setDoOutput(true);c.setRequestProperty("Content-Type","application/json");c.setRequestProperty("Authorization","Bearer "+secret);
                JSONObject req=new JSONObject();req.put("action",action);req.put("payload",payload==null?new JSONObject():payload);byte[] b=req.toString().getBytes(StandardCharsets.UTF_8);String ts=String.valueOf(System.currentTimeMillis()/1000L);String nonce=UUID.randomUUID().toString();String sig=hmacHex(secret,ts+"\n"+nonce+"\n"+new String(b,StandardCharsets.UTF_8));c.setRequestProperty("X-PRD-Timestamp",ts);c.setRequestProperty("X-PRD-Nonce",nonce);c.setRequestProperty("X-PRD-Signature",sig);c.setFixedLengthStreamingMode(b.length);try(OutputStream o=c.getOutputStream()){o.write(b);}int code=c.getResponseCode();BufferedReader r=new BufferedReader(new InputStreamReader(code>=200&&code<300?c.getInputStream():c.getErrorStream(),StandardCharsets.UTF_8));StringBuilder s=new StringBuilder();String line;while((line=r.readLine())!=null)s.append(line);JSONObject body=s.length()>0?new JSONObject(s.toString()):new JSONObject();if(cb!=null)cb.onResult(code>=200&&code<300,body.optString("message","HTTP "+code),body);
